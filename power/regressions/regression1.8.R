mainreg = function (){
        ...
        cork <- cor(df.pmpowers$power, df.pmpowers$pm10, use = "complete.obs")
        ...
        ginv = g + aes(power, pm10) + geom_point(alpha = 1/3) + geom_smooth(method="lm") 
        title = paste("Regressions in", input$year)
        ...
        fname = paste("regressions",as.character(input$year),".pdf",sep="")
        if (dbg) browser()
        ggsave(ginv, file=fname)
}
