...
source(str_glue(path_to_source, "tidySounding.R"))
path_to_data <- "/home/kwabena/Documents/trafin/lovy/soundings/src/test/"

describe("tidySounding.R", {
    ...
    describe("(getVirtualCins)", {
        ...
        it("expects to return the right value for the Virtual CINS Index", {
            expect_equal( getVirtualCins(text), -6.00)
    ...
    describe("(getHtmlDocAndHeaders",{
        file_url <- str_glue("{path_to_data}/78526_2016_04.html")
        head_strings <- getHtmlDocAndHeaders(fileurl = file_url)
        expect_equal( length(head_strings), 2)
        expect_equal( length(head_strings[[2]]), 59)
    })
    
    describe("(getDfOfThreeColumns)", {
        file_url <- str_glue("{path_to_data}/78526_2016_04.html")
        doc_with_head_strings <- getHtmlDocAndHeaders(fileurl = file_url)
        df_of3 <- getDfOfThreeColumns(doc_with_head_strings)
        expect_equal(length(df_of3$data_string), 30)
        expect_equal(length(df_of3$context_string), 30)
        expect_equal(length(df_of3$head_string), 30)
