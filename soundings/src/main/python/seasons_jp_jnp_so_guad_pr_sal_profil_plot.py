...
karu_jnp_filename = os.path.join('../sql/tephi', 'guadeloupe_jnp_so_tephi_data.txt')
karu_jp_filename =  os.path.join('../sql/tephi', 'guadeloupe_jp_so_tephi_data.txt')

pr_jp_filename = os.path.join('../sql/tephi', 'puerto_rico_jp_so_tephi_data.txt')
...
tpg.plot(karu_jp_temps, label='Guadeloupe JP SO temperature', color='red', linewidth=2,linestyle='-', marker='s')

tpg.plot(karu_jnp_temps, label='Guadeloupe JNP SO temperature', color='magenta', linewidth=2,linestyle='--', marker='s')

tpg.plot(pr_jp_temps, label='Porto-Rico JP SO temperature', color='blue', linewidth=2,linestyle='-', marker='s')

tpg.plot(pr_jnp_temps, label='Porto-Rico JNP SO temperature', color='cyan', linewidth=2,linestyle='--', marker='s')

tpg.plot(sal_temps, label='SAL Dunion temperature', color='black', linewidth=2, linestyle='-', marker='o')

plt.savefig('seasons_so_jp_guad_pr_temp.jpg')
