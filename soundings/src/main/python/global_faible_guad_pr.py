...
karu_filename = os.path.join('../sql/tephi', 'guadeloupe_faible_tephi_data.txt')
pr_filename = os.path.join('../sql/tephi', 'puerto_rico_faible_tephi_data.txt')
...
tpg.plot(karu_temps, label='Guadeloupe faible temperature', color='red', linewidth=2,linestyle='-', marker='s')

tpg.plot(pr_temps, label='Porto-Rico faible temperature', color='blue', linewidth=2,linestyle='-', marker='s')

tpg.plot(sal_temps, label='SAL Dunion temperature', color='black', linewidth=2, linestyle='-', marker='o')

plt.savefig('global_faible_guad_pr_temp.jpg')
