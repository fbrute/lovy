...
class Hymarch22:
  def __init__(self:'Hymarch22', scenario_name:'str', station):
  ...
  def getShpFiles(self, path):
    shp_files = list(path.glob(f"{self.getStation()}*.shp"))
    return(shp_files)
  ...
  def saveProject(self):
    path = self.getPath() /  f"{self.getStation()}_{self.getPath().name}.qgs"
    print(f"path = {path}")
...
def main():
  station = input("Station? (barb, karu, or puer")
  ...
  hy = Hymarch22(scenario_name=scenario_name, station=station)
