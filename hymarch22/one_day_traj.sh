...
if [[ ${#} -lt 4 ]]; then
  echo 'Please check that you provided the station(where), altitude, date format (ymd or mdy) and date, mode (test or prod)'
  exit
fi
...
# Set Project home (source folder)
...
if [[ $mode = 'prod' ]]; then
  ...
  if [[ $mode = 'test' ]]; then
  result_path=$METROOT/tests/retros_mars_2018/${where}_$olvl
    if ! [[ -d $result_path ]]; then 
...
print "The dateformat is ${date_format}"
